...
describe('LogicEncoder Use Cases', () => {
    it("should encode a single course requirement correctly", () => {
        ...
        const courseA = new Course('CS101', [], 3);
        courseRepository.addCourse(courseA);
        const req = new Requirement([courseA], 1); // Requires CS101
        const major = new Major([], [req], 'Computer Science', 'Engineering');
        const solver = new Logic.Solver();
        const encoder = new LogicEncoder(solver, courseRepository);
        encoder.encodeMajor(major);
        expect(solver.solve()).not.toBeNull(); // Should have a solution
    });

    it('should encode a simple or requirement', async () => {
        // Mock data
        const courseRepository = new MockCourseRepository();
        const courseA = new Course('CS101', [], 3);
        const courseB = new Course('CS102', [], 3);
        courseRepository.addCourse(courseA);
        courseRepository.addCourse(courseB);
        ...
        const encoder = new LogicEncoder(solver, courseRepository);
        // Encode the major
        await encoder.encodeMajor(major);
        expect(solver.solve()).not.toBeNull(); // Should have a solution
    ...
    it('should encode a track with multiple requirements', async () => {
        // Mock data
        const courseRepository = new MockCourseRepository();
        const courseA = new Course('CS101', [], 3);
        const courseB = new Course('CS102', [], 3);
        const courseC = new Course('CS103', [], 3);
        courseRepository.addCourse(courseA);
        courseRepository.addCourse(courseB);
        courseRepository.addCourse(courseC);
    ...
    it("should encode a major with both tracks and standalone requirements", async () => {
        // Mock data
        const courseRepository = new MockCourseRepository();
        const courseA = new Course('CS101', [], 3);
        const courseB = new Course('CS102', [], 3);
        const courseC = new Course('CS103', [], 3);
        const courseD = new Course('CS104', [], 3);
        courseRepository.addCourse(courseA);
        courseRepository.addCourse(courseB);
        courseRepository.addCourse(courseC);
        courseRepository.addCourse(courseD);
        const req1 = new Requirement([courseA], 1); // Requires CS101
        const req2 = new Requirement([courseB, courseC], 1); // Requires at least one of CS102 or CS103
        const track = new Track([req1, req2], 'Software Engineering Track', "Engineering");
    ...
    it('should handle multiple majors correctly', async () => {
        ...
        const courseC = new Course('MATH101', [], 3);
        const courseD = new Course('MATH102', [], 3);
        ...
        const encoder = new LogicEncoder(solver, courseRepository);
        // Encode both majors
        await encoder.encodeMajor(major1, solver);
        await encoder.encodeMajor(major2, solver);
        expect(solver.solve()).not.toBeNull(); // Should have a solution
        const solution = solver.solve();
        if (solution !== null) {
    ...
    it('should handle edge case of no tracks and no requirements', async () => {
        const courseRepository = new MockCourseRepository();
    ...
    it("should handle prerequisites correctly", async () => {
        ...
        const prereq = new Prerequisite([courseA.coursecode]); // CS101 is a prerequisite
        const courseB = new Course('CS102',[prereq], 3); // CS102 has a prerequisite of CS101
        await courseRepository.addCourse(courseB);
    ...
    it("should handle nested prerequisites correctly", async () => {
        ...
        const courseRepository = new MockCourseRepository();
        const courseA = new Course('CS101', [], 3);
        const prereq1 = new Prerequisite([courseA.coursecode]); // CS101 is a prerequisite
        const courseB = new Course('CS102', [prereq1], 3); // CS102 has a prerequisite of CS101
        const prereq2 = new Prerequisite([courseB.coursecode]); // CS102 is a prerequisite
        const courseC = new Course('CS103', [prereq2], 3); // CS103 has a prerequisite of CS102
        const req = new Requirement([courseC], 1); // Requires CS103

        await courseRepository.addCourse(courseA);
        await courseRepository.addCourse(courseB);
        await courseRepository.addCourse(courseC);
